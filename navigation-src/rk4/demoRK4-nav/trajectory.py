...
"""
def approximate(initial_state) :#w, M) :
    ...
    while G and (time_t-time_0) < time_limit :
...
def plot_computed_motions(new_state):
...
def test_framework() :
    ...
    set_potential_field_trajectory(initial_state,target_state)
    ...
    seq_fin = approximate(initial_state)
    
    t_2 = time.time()

    
    print "> Etats résultant des primitives de mouvement précédentes"
    plot_computed_motions(initial_state)
