...
def main() :
    ...
    for T in T_discretisation_sets :
        ...
        plt.figure("Variation de u pour différentes horizons avec un pas de "+str(T_discretisation)+"s")
        i_horizon = 0
        for U in U_results :
            plt.plot(range(0,len(U)), U, label='$ T_{horizon} = '+str(horizon_iteration_set[i_horizon])+'$')
            i_horizon += 1
        plt.legend()
