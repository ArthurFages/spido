...
horizon_iteration   = 7        # nombre d'itérations à considérer dans la commande prédictive
...
Cr      = 110 # rigidité de dérive du train arrière
M       = 900
Iz      = 1700
a       = 1.0
b       = 1.0
...
thau_rk = 0.005 # à faire dépendre de T_discretisation
...
def set_maneuver_0(horizon_iteration,T_discretisation) :
    ...
    states = []
    size = horizon_iteration*5 # on fixe la longueur de la primitive sur la longueur de la prédiction
    t = 0
    tf = (size+1)*T_discretisation # Vx m.s-1 => Vx m en 1s
    psi = np.radians(0)
...
primitive_from_file("virage_gauche_court_10-gazebo",3)

# chargement virages à droite
primitive_from_file("virage_droite",2)
primitive_from_file("virage_droite_court_10-gazebo",4)
