...
# == Drapeaux pour les gestions des process parrallèles =========================
...
def initiate_ref_signal(ref_starting_state, horizon_iteration, id_maneuver) :

    global maneuvers

    # détermination de la trajectoire de référence
    print "\n[*] Définition d'une suite de primitives de mouvement"
    seq_fin = [maneuvers[id_maneuver].copy()]

    print "\n[*] Définition des trajectoires de référence"
...
def publish_command(u,id_proc) :

    global T_discretisation
    global STOP_PUBLISH_COMMAND

    STOP_PUBLISH_COMMAND = True # arrêt des autres fonctions de publication de commande
    time.sleep(0.3) # microtemporisation
    STOP_PUBLISH_COMMAND = False # la fonction courante à desormais le droit de publier et est la seule à le faire 
    ...
    for i in range(1,len(u)) :
        ...
        if STOP_PUBLISH_COMMAND :
            print str(id_proc)+" > OK j'arrête"
            return

        print str(id_proc)+" > publish command : ("+str(u[i])+" ; "+str(u[i])+")"
        bf = float(u[i])
        br = -float(u[i])
        front_r_steer_pub.publish(bf)
        front_l_steer_pub.publish(bf)
        rear_r_steer_pub.publish(br)
        rear_l_steer_pub.publish(br)
        time.sleep(T_discretisation)

    print "[!] No more command to publish for "+str(id_proc)
...
def test_gazebo() :
    ...
    u_ref, x_ref, t_ref, Xref, Yref = initiate_ref_signal(current_state, horizon_iteration*5, 0)

    print "len X_ref : ",len(Xref)
    
    print "\n[*] Suivi de ces trajectoires par MPC sous gazebo"
    for i in range(0,len(Xref)) :
        ...
        u, duration = get_commands_on_horizon(T_discretisation,horizon_iteration,current_state,current_command,u_ref, x_ref, t_ref, Xref, Yref) 
        times_results.append(duration)
        print "> new command : "+str(u)

        # envoie de la commande au spido
        publish_thread = threading.Thread(None, publish_command, None, (u,i)) 
    ...
    plt.plot(X_results, Y_results, label="Results")
    plt.legend()
    ...
    erreur_X = X_results - Xref
    erreur_Y = Y_results - Yref

    # affichage des erreurs
    plt.figure("Erreur selon x")
    plt.plot(range(0,len(erreur_X)),erreur_X)
    plt.figure("Erreur selon y")
    plt.plot(range(0,len(erreur_Y)),erreur_Y)
