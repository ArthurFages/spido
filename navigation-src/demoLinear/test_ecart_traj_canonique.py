...
def get_commands_on_horizon(T_discretisation,horizon_iteration,current_state, current_command,u_ref, x_ref, t_ref, Xref, Yref) :
    ...
    t1 = time.time()
    # options={'maxiter': 20},
    var_optimized = minimize(fun=f_cost_MPC,x0=var_0, bounds=bnds, constraints=cons, args=[u_ref,x_ref]).x
...
def main() :
    ...
    # valeurs à tester
    T_discretisation_sets   = [0.1,0.15,0.2,0.3]
    horizon_iteration_set   = [5,10,15,20,30]
    ...
    for T in T_discretisation_sets :
        ...
        for horizon in horizon_iteration_set :
            ...
            for commande in u :
                Uf.append(commande[0])
                Ur.append(commande[1])

            U_results.append(u)
            Uf_results.append(Uf)
            Ur_results.append(Ur)
            
            print "u : ",len(u)
            print "Uf : ",len(Uf_results)
            print "Ur : ",len(Ur_results)
            print "u_ref : ",len(u_ref)

        # affichage des résultats
        plt.figure("Variation de U pour différentes horizons avec un pas de "+str(T_discretisation)+"s",figsize=(10, 10))
        ref = []
        for commande_ref in u_ref :
            ref.append(commande_ref[0])
        plt.plot(range(0,len(ref)), ref, ':', label='$Ref$')
        #plt.step(range(0,len(Uf_results)),Uf_results)
        #plt.step(range(0,len(Ur_results)),Ur_results)
        i_horizon = 0
        for i in range(0,len(Uf_results)) :
            plt.step(range(0,len(Uf_results[i])),Uf_results[i], label="$front : T_{horizon} = "+str(horizon_iteration_set[i_horizon])+'$')
            plt.step(range(0,len(Ur_results[i])),Ur_results[i], label="$rear : T_{horizon} = "+str(horizon_iteration_set[i_horizon])+'$')        
            #plt.plot(range(0,len(Uf_results[i])), Uf_results[i], label='$ T_{horizon} = '+str(horizon_iteration_set[i_horizon])+'$')
            #plt.plot(range(0,len(Ur_results[i])), Ur_results[i])
            i_horizon += 1
        plt.ylabel(ur"$\beta \, (deg)$", fontsize=16)           
        plt.xlabel(ur"$Horizon \, (nb\ pas\ de\ discretisation)$", fontsize=16)
        plt.legend()
        plt.savefig('U-'+str(T_discretisation)+'.jpg')
        plt.show()
        ...
        for Y in Y_results :
            ...
            plt.ylabel(ur"$Erreur \, (m^{2})$", fontsize=16)           
            plt.xlabel(ur"$Horizon \, (nb\ pas\ de\ discretisation)$", fontsize=16)
