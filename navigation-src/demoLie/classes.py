...
class Obstacle :
    def __init__(self, x, y, r=0) :
        ...
        self.r = r
    def to_string(self) :
        return "Obstacle - x "+str(self.x)+" - y "+str(self.y)+" - r "+str(self.r)
...
class Node2 :
    """
    state : robot state for this node
    prec : previous node
    fol : états possibles depuis l'état courant, classé du plus proche de l'état final au moins proche
    id_fol : états suivant choisis dans fol
    u : command to reach this node from the previous node
    """
    def __init__(self, state, prec, fol, id_fol, u) :
        self.state  = state
        self.prec   = prec
        self.fol    = fol
        self.id_fol = id_fol
        self.u      = u
    def to_string(self) :
        return "(",self.state.to_string_v()#,") atteint par : ",self.u
