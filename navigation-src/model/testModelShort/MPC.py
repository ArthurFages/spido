...
def get_t_ref(current_position, states_ref) :
...
def get_x_ref(current_position, states_ref) :
...
def get_u_ref(current_position, states_ref) :
...
def f_cost_MPC(U,args):#current_state,current_command) :
    ...
    # génération d'indices pairs
    for i in range(0,2*(horizon_iteration-1)) :
        if i%2==0 :
            range_i.append(i)

    # mise en forme de U
    for i in range_i :
        well_formatted_U.append([U[i],U[i+1]])
    U = np.array(well_formatted_U,dtype=float_)
    
    # initialisation
    f_cost  = 0
    
    #x_i     = np.array([stable_current_state.Vy, stable_current_state.Vpsi, stable_current_state.psi, stable_current_state.x, stable_current_state.y],dtype=float_)
    x_i     = np.array([stable_current_state.Vpsi, stable_current_state.Vx, stable_current_state.Vy, stable_current_state.psi, stable_current_state.x, stable_current_state.y],dtype=float_)
    u_i     = np.array([current_command.betaf, current_command.betar],dtype=float_)
    
    x_ref_i = np.array(x_ref[0],dtype=float_)
    u_ref_i = np.array(u_ref[0],dtype=float_)
    #T       = t_ref[0]
    ...
    A   = get_model_a2_matrix_lin(stable_current_state)
    ...
    #for i in range(1,len(t_ref)+1):
    for i in range(1,len(U)):

        delta_state = np.matrix([x_i[3],x_i[4],x_i[5]])-np.matrix([x_ref_i[3],x_ref_i[4],x_ref_i[5]])
        #delta_state = np.matrix([x_i[4],x_i[5]])-np.matrix([x_ref_i[4],x_ref_i[5]])
        deltas.append(delta_state[0,1])
        #print "x_i : ",x_i

        f_cost = f_cost + 1/2*(Q*(delta_state).dot(delta_state.transpose()) + R*(u_i-u_ref_i).dot(u_i-u_ref_i) + S*(U[i]-u_i).dot(U[i]-u_i))
        
        # prédiction de l'état selon le modèle dynamique
        # vérifier le sens des beta (peut être source d'erreur)
        
        
        # définition du modèle dynamique discret en fonction du temps d'échantillonage
        # linéarisation au points de fonctionnement actuel
        # puis discrétisation sur un pas de temps inhérents aux trajectoires de référence
        #t1 = time.time()

        # tant que l'horizon de prédiction n'est pas atteint, on calcule l'état suivant
        if i<len(t_ref)-1 :
            ...
            if np.isnan(np.sum(A)) :
                print "NaN in A"
            if np.isnan(np.sum(Ad)) :
                print "NaN in Ad"
            if np.isnan(np.sum(B)) :
                print "NaN in B"
            if np.isnan(np.sum(Bd)) :
                print "NaN in Bd"

            #print "Model computation time : ",(t2-t1)
            ...
            if not type(Ad) == type(0) and not type(Bd) == type(0) :
                ...
                print "[!] Error Ad and Bd calculation"
                #print "Ad : ",Ad
                #print "A : ",A
                print "Bd : ",Bd
                print "B : ",B
            ...
            # ELSE : problème de calcul de la discretisation 
            # TODO : explain mudafucka
    ...
    #print "len U : ",len(U)
    #print "len tref : ",len(t_ref)
    #print "deltas y MPC : ",deltas
    #print "time prediction  : ",t_ref[-1]-t_ref[0]
    #print "time interval    : ",t_ref[1]-t_ref[0]
    #print "f_cost output : ",f_cost
...
def test_minimum_f_cost() :
    ...
    t1 = time.time()
    u_optimized = minimize(fun=f_cost_MPC, x0=u_0).x#, accept_test=mybounds, niter=1).x
    t2 = time.time()
